...
`RpcRouter` builds on top of [`RpcClient`](/docs/api/rpc-client). Every higher-level SDK module that needs resilient RPC access — such as `TransactionSimulator` — uses a router rather than a bare client.

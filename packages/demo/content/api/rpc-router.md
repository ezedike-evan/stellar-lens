---
title: RpcRouter
description: Resilient connection pool that routes calls to the fastest healthy Soroban RPC endpoint.
---
...
`RpcRouter` builds on top of [`RpcClient`](/docs/api/rpc-client). Every higher-level SDK module that needs resilient RPC access — `TransactionSimulator`, `FeeEstimator` — uses a router rather than a bare client.
